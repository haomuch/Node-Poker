...
function cleanupInactiveRooms(){
  for(const roomCode in rooms){
    ...
    if(connectedPlayers === 0 && (Date.now() - table.lastActive > ROOM_INACTIVE_TIMEOUT)){
      ...
      if(table.pendingNext) clearTimeout(table.pendingNext);
...
function goShowdown(table){
  ...
  const showdownPlayers = table.players.filter(p => p.inHand && !p.folded);
  ...
  // 2. 准备一个玩家下注信息列表，并按投入从低到高排序
  const playerContributions = showdownPlayers.map(p => ({
      playerId: p.playerId,
      committed: p.totalCommitted,
      chips: p.chips,
      seat: p.seat,
      rank: ranks[p.playerId]
  ...
  // 4. 构建池子并分配奖金（修复：仅多边池才分配奖金和推进 processedCommitment）
  for(let i = 0; i < playerContributions.length; i++) {
      const currentLevel = playerContributions[i].committed - processedCommitment;
      if (currentLevel <= 0) continue;

      const eligiblePlayers = playerContributions.slice(i);
      
      if (eligiblePlayers.length > 1) {  // 新增：仅当多人 eligible 时才创建 pot
          const potSize = currentLevel * eligiblePlayers.length;

          // 找出该池子的赢家
          let bestRank = null;
          let potWinners = [];
          for(const eligible of eligiblePlayers) {
          ...
          pots.push({
              amount: potSize,
              eligibles: new Set(eligiblePlayers.map(p => p.playerId)),
              winners: potWinners.map(p => p.playerId)
          });
          
          processedCommitment += currentLevel;  // 仅在这里推进
      }
      // else: 单人 eligible（超额返还），跳过：不分配、不推进 processedCommitment，让 refund 处理
  }
  
  // 5. 将筹码加到玩家的账户并设置 lastAction
  for(const p of table.players){
      const wonAmount = winningAmounts.get(p.playerId) || 0;
      
      // 检查超额下注返还（现在包括单人“池子”部分）
      ...
      p.lastAmount = wonAmount; // 只有赢得的金额才会显示（不包括 refund）
      
      // 判断玩家是否赢得任何筹码（仅基于 wonAmount，即多边池）
      if (wonAmount > 0) {
          p.lastAction = "WIN";
      }
      // 否则，保持其之前的状态
...
function tryStartHand(table){
  ...
  if(table.dealerSeat < 0) table.dealerSeat = ready[0].seat;
    ...
    const nd = nextSeatFrom(table, table.dealerSeat, p=>p.connected && p.chips>0 && !table.rebuyPending.has(p.id));
    if(nd) table.dealerSeat = nd.seat;
  }

  // initialize players; reset reveal flag
  for(const p of table.players){
    if(p.connected && p.chips > 0 && !table.rebuyPending.has(p.id)){
...
io.on("connection", socket=>{
  ...
  socket.on("join", ({ name, room, playerId }) => {
    ...
    if (ghost) {
      ...
      // 更新 currentToAct 如果匹配 oldId
      if (table.currentToAct === oldId) {
        table.currentToAct = socket.id;
      }

      // 更新 pendingActors 中的 oldId
      table.pendingActors = table.pendingActors.map(pid => pid === oldId ? socket.id : pid);

      // 更新 rebuyPending
      if (table.rebuyPending.has(oldId)) {
        table.rebuyPending.delete(oldId);
        table.rebuyPending.add(socket.id);
      }

      // 更新 sidePots.eligibles
      if (table.sidePots) {
        for (const sp of table.sidePots) {
          if (sp.eligibles.has(oldId)) {
            sp.eligibles.delete(oldId);
            sp.eligibles.add(socket.id);
          }
        }
      }
